...
- **맵의 특징**
  - 각 노드가 key와 value 쌍으로 이루어진 컨테이너 (이진 탐색 트리, first-second가 있는 pair 객체로 저장)
  - 중복된 key 값을 사용할 수 없음 (같은 key 값을 갖는 원소를 저장하고 싶다면 multimap 이용)
  - 인덱스 접근 불가능 (다만 key를 이용해 접근은 가능함 → m(key))
  - 원소가 오름차순으로 자동 정렬됨 (이진 탐색 트리로 구현되어 있기 때문)
  - 정렬된 상태로 key-value 자료 저장을 하고 싶을 때 사용함
...
```
- **해시맵의 특징**
  - 각 노드가 key와 value 쌍으로 이루어진 컨테이너 (map과 동일하게 중복된 key 사용불가))
  - 인덱스 접근 불가능 (다만 key를 이용해 접근은 가능함 → hm(key))
  - 원소가 자동으로 정렬되지 않고 연산이 빠름 (해시 테이블로 구현되어 있기 때문)
  - 정렬이 필요없으며 빠른 연산을 원할 때 사용함
  - key-value 쌍으로 저장되는 특성에 따라 자주 사용됨 (코딩테스트 문제풀이에서 많이 이용되는 자료구조)
...
- **Header**: `#include <queue>` (우선순위 큐) (힙은 STL이 제공되지 않음, vector에 저장된 값을 힙 구조로 조정하는 함수는 `#include <algorithm>`에 있음)
- **힙의 특징**
  - 완전 이진 트리 기반의 자료구조
  ...
  - 배열에서 인덱스가 i라면 왼쪽자식 노드의 인덱스는 2i+1, 오른자식 노드의 인덱스는 2i+2, 부모노드의 인덱스는 (i-1)/2
  - 우선순위를 고려하는 문제에서 사용됨 (최댓값 혹은 최솟값을 빠르게 찾을 수 있음)
...
```
- **우선순위 큐의 특징**
  - 우선순위 큐는 일반적인 큐와 달리 들어간 순서에 상관없이 우선순위가 높은 데이터가 먼저 나옴
  - 최대힙(내림차순)이 기본구조 (설정을 추가하여 정렬 기준을 변경할 수 있음)
